import '../../../core/modules/constants/constants/string_const.dart';
import '../../../core/modules/exceptions/exceptions/firebase_exceptions.dart';
import '../../../core/modules/exceptions/exceptions/platform_exceptions.dart';
import '../../../core/modules/logging/logger/app_logger.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:firebase_core/firebase_core.dart';
import 'package:firebase_storage/firebase_storage.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/services.dart';

class FirebaseRepo {
  FirebaseRepo._();
  static final FirebaseRepo instance = FirebaseRepo._();

  final FirebaseAuth _auth = FirebaseAuth.instance;
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  FirebaseFirestore get firestore => _firestore;

  User? get currenuser => _auth.currentUser;

  bool get isAuthenticated => currenuser != null;

  ////HEre wrote auth codes
  ///signinwithemailandpassword
  signin(String email, String password) async {
    try {
      AppLogger.d("signIn method started with email: $email");
      UserCredential userCredential = await FirebaseAuth.instance
          .signInWithEmailAndPassword(email: email, password: password);
      AppLogger.d("signIn method completed");
      return userCredential;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository signIn error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  ///register with email and password
  registerWithEmailandPassword(String email, String password) async {
    try {
      UserCredential userCredential = await _auth
          .createUserWithEmailAndPassword(email: email, password: password);

      return userCredential;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository registerWithEmail error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  sendVerificationEmail() async {
    try {
      AppLogger.d('Attempting to send email verification');
      await FirebaseAuth.instance.currentUser!.sendEmailVerification();
      AppLogger.d('Email verification sent successfully via FirebaseAuth');
    } on FirebaseException catch (e) {
      AppLogger.e('FirebaseException sending verification email: ${e.code}');
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      AppLogger.e('PlatformException sending verification email: ${e.code}');
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e(
        'AuthRepository sendVerificationEmail error: ${e.toString()}',
      );
      if (kDebugMode) {
        print(e.toString());
      }
      throw e.toString();
    }
  }

  ///signinaninomusly
  signinanynomusly() async {
    try {
      final UserCredential userCredential = await _auth.signInAnonymously();
      return userCredential.user!.uid;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository signinanynomusly error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  //signout
  void signOut() async {
    try {
      await _auth.signOut();
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository signOut error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  ///data storage code

  ///create collection and add data
  Future<DocumentReference<Map<String, dynamic>>> createCollection(
    String collectionName,
    Map<String, dynamic> data, {
    String? docId,
  }) async {
    try {
      final CollectionReference<Map<String, dynamic>> collectionRef = _firestore
          .collection(collectionName);

      DocumentReference<Map<String, dynamic>> docRef;
      if (docId != null) {
        //  custom document ID
        docRef = collectionRef.doc(docId);
        await docRef.set(data);
      } else {
        // Auto-generate the document ID
        docRef = await collectionRef.add(data);
      }

      return docRef;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository createCollection error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
      throw e.toString();
    }
  }

  //create subcollection data
  createSubCollection(
    String collectionName,
    String subCollectionName,
    dynamic docId,
    dynamic subDocId,
    Map<String, dynamic> data,
  ) async {
    try {
      final docref = await _firestore
          .collection(collectionName)
          .doc(docId)
          .collection(subCollectionName)
          .doc(subDocId)
          .set(data);
      return docref;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository createSubCollection error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  //filtter data
  filterUsingQuery(
    String collectionName,
    String fieldName,
    dynamic value,
  ) async {
    try {
      final data = await _firestore
          .collection(collectionName)
          .where(fieldName, isEqualTo: value)
          .get();
      return data;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      AppLogger.e('AuthRepository filterUsingQuery error: ${e.toString()}');
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  ///read data from database using name and id
  Future<DocumentSnapshot<Map<String, dynamic>>>
  readDataUsingIdandColletionname(String collectionName, dynamic id) async {
    try {
      final docref = await _firestore.collection(collectionName).doc(id).get();
      return docref;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
    }
    throw 'Something went wrong';
  }

  fetchdatafromattendance(dynamic date) async {
    try {
      final data = await FirebaseFirestore.instance
          .collection(StringConst.userdata)
          .doc(currenuser!.uid)
          .collection(StringConst.attendancerecord)
          .doc(date)
          .get();
      return data;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
    }
  }

  ///read sub collection

  Future<QuerySnapshot<Map<String, dynamic>>>
  readSubCollectionDataUsingIdandColletionname(
    String collectionName,
    dynamic docId,
    String subCollectionName,
  ) async {
    try {
      final docref = _firestore
          .collection(collectionName)
          .doc(docId)
          .collection(collectionName)
          .get();
      return docref;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(e.code).message;
    } on PlatformException catch (e) {
      throw PlatformExceptionString(e.code).message;
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
    }
    throw 'Something went wrong';
  }
  //read data from subcollection using qury
  //   Future<QuerySnapshot<Map<String, dynamic>>>
  //     readSubCollectionUsingquery(
  //         String collectionName, dynamic id, String subCollectionName) async {
  //   try {
  //     final docref = await _firestore
  //         .collection(collectionName)
  //         .doc(id)
  //         .collection(collectionName);
  //     return docref;
  //   } on FirebaseException catch (e) {
  //     throw FirebaseExceptionString(e.code).message;
  //   } on PlatformException catch (e) {
  //     throw PlatformExceptionString(e.code).message;
  //   } catch (e) {
  //     if (kDebugMode) {
  //       print(e.toString());
  //     }
  //   }
  //   throw 'Something went wrong';
  // }
  ///read all data from database using collection

  Future<List<QueryDocumentSnapshot<Map<String, dynamic>>>> readAllDocuments(
    String collectionName,
  ) async {
    try {
      final querySnapshot = await _firestore.collection(collectionName).get();
      return querySnapshot.docs;
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } on PlatformException catch (e) {
      throw PlatformExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw 'An unexpected error occurred.'; // Or provide a more user-friendly message
    }
  }

  ///update data
  updateData(
    String collectionName,
    dynamic id,
    Map<String, dynamic> data,
  ) async {
    try {
      _firestore.collection(collectionName).doc(id).update(data);
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } on PlatformException catch (e) {
      throw PlatformExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw 'An unexpected error occurred.'; // Or provide a more user-friendly message
    }
  }

  Future<void> updateSubcollectionData(
    String collectionName,
    dynamic id,
    dynamic subcollectionId,
    String subCollectionName,
    Map<String, dynamic> data,
  ) async {
    try {
      // Reference to the subcollection
      _firestore
          .collection(collectionName)
          .doc(id)
          .collection(subCollectionName)
          .doc(subcollectionId)
          .update(data);
    } on FirebaseException catch (e) {
      throw Exception('Firebase error: ${e.code} - ${e.message}');
    } on PlatformException catch (e) {
      throw Exception('Platform error: ${e.code} - ${e.message}');
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw Exception('An unexpected error occurred.');
    }
  }

  ///delete doc
  deleteDate(String collectionName, dynamic id) async {
    try {
      await _firestore.collection(collectionName).doc(id).delete();
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } on PlatformException catch (e) {
      throw PlatformExceptionString(
        e.code,
      ).message; // Or handle the error as needed
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw 'An unexpected error occurred.'; // Or provide a more user-friendly message
    }
  }
  //cloud storage

  Future<String> uploadFile(Uint8List data, String path) async {
    if (!isAuthenticated) {
      throw 'User not authenticated';
    }
    try {
      final TaskSnapshot snapshot = await FirebaseStorage.instance
          .ref(path)
          .putData(data);
      return await snapshot.ref.getDownloadURL();
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(
        e.code,
      ).message; // Or handle errors as needed
    } on PlatformException catch (e) {
      throw PlatformExceptionString(
        e.code,
      ).message; // Or handle errors as needed
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw 'An unexpected error occurred.'; // Or provide a user-friendly message
    }
  }

  // Add this function to compress the image (replace with your logic if needed)
  deleteFile(String path) async {
    try {
      await FirebaseStorage.instance.ref(path).delete();
    } on FirebaseException catch (e) {
      throw FirebaseExceptionString(
        e.code,
      ).message; // Or handle errors as needed
    } on PlatformException catch (e) {
      throw PlatformExceptionString(
        e.code,
      ).message; // Or handle errors as needed
    } catch (e) {
      if (kDebugMode) {
        print(e.toString());
      }
      throw 'An unexpected error occurred.'; // Or provide a user-friendly message
    }
  }

  readDataUsingIdAndCollectionName(String attendancerecord, String s) {}
}
